...
Mqq=Gint'*IFP*Gint;
Qv1=cross(omegaint,IFP*omegaint)+IFP*G2*betasd';
Qv=-Gint'*Qv1;
Qe=Gint'*TFP;
...
Mat = [Mqq Cq';
       Cq 0];
Forces=[Qv+Qe;-2.*b1d*b1d-2.*b2d*b2d-2.*b0d*b0d-2.*b3d*b3d ];
NewRes=Mat\Forces;

omegadot = Gint*NewRes(1:4) + G2*betasd';
...
second_derivatives = [a_kin; NewRes(1:4)];
Consts=[x;y;z]+A*sj;
tempC = [Consts;
    b0.^2+b1.^2+b2.^2+b3.^2-1];
C=tempC(1:4);

dC=[[xd;yd;zd]-v_kin; 2*b0*b0d+2*b1*b1d+2*b2*b2d+2*b3*b3d];
ddC=[0;0;0;0];
...
function [t, F, second_derivatives,C,dC,ddC,Ener,UC,theta,omegaint] = custom_rk4(odefun, tspan, y0)
    ...
    for i = 1:(n-1)
        ...
        [F_temp(7:10,i+1), F_temp(11:14,i+1)] = renormalize_quaternion( ...
            F_temp(7:10,i+1), F_temp(11:14,i+1));
...
if ~exist('save_filename','var') || isempty(save_filename)
    save_filename = sprintf('UncHT_ClassicEP_dt_%.2fms.mat', dt*1000);
end
